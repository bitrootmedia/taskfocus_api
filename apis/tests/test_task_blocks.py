...
class TaskBlocksTestsV2(APITestCase):
    ...
    def test_task_block_list_no_task_access(self):
        ...
        response = self.client.get(
            reverse(
                "task_block_list_v2", kwargs={"task_id": str(self.task_1.id)}
            )
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_block_list_ordered(self):
        ...
        response = self.client.get(
            reverse(
                "task_block_list_v2", kwargs={"task_id": str(self.task_3.id)}
            )
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        results = response.json().get("results")
    ...
    def test_block_list_empty(self):
        ...
        response = self.client.get(
            reverse(
                "task_block_list_v2", kwargs={"task_id": str(self.task_4.id)}
            )
        )
        results = response.json().get("results")
    ...
    def test_block_update_partial_invalid(self):
        ...
        self.client.post(
            reverse(
                "task_block_list_v2", kwargs={"task_id": str(self.task_2.id)}
            ),
            data=[
                {
                    "id": self.block_2.id,
