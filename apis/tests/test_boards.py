...
from core.models import Board, BoardUser, Card, CardItem, Project, Task, User
...
class BoardTest(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.board_user_4 = BoardUser.objects.create(board=cls.board, user=cls.user_4)
    ...
    def test_board_list_search(self):
        self.client.force_login(user=self.user)
        response = self.client.get(reverse("board_list") + f"?name={self.board.name}")
        board_ids = [x["id"] for x in response.json()["results"]]
    ...
    def test_board_detail_get_not_owner(self):
        ...
        response = self.client.get(reverse("board_detail", kwargs={"pk": self.board.id}))
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_board_detail_edit_not_owner(self):
        self.client.force_login(user=self.user_2)
        response = self.client.put(reverse("board_detail", kwargs={"pk": self.board.id}))
    ...
    def test_board_detail_delete_not_owner(self):
        self.client.force_login(user=self.user_2)
        response = self.client.delete(reverse("board_detail", kwargs={"pk": self.board.id}))
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_board_user_get_users(self):
        ...
        response = self.client.get(reverse("board_users", kwargs={"board_id": self.board.id}))
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_board_users_get_no_users(self):
        self.client.force_login(self.user_2)
        response = self.client.get(reverse("board_users", kwargs={"board_id": self.board_3.id}))
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_board_user_create_not_owner(self):
        self.client.force_login(self.user_2)
        response = self.client.post(reverse("board_users", kwargs={"board_id": self.board.id}))
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_board_user_delete_not_owner(self):
        self.client.force_login(self.user_2)
        response = self.client.delete(reverse("board_users", kwargs={"board_id": self.board.id}))
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_card_delete_not_empty(self):
        self.client.force_login(self.user)
        response = self.client.delete(reverse("card_detail", kwargs={"pk": self.card.id}))
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertEqual(
    ...
    def test_card_move(self):
        self.client.force_login(self.user)
        response = self.client.put(reverse("card_move"), {"card": self.card.id, "position": 1})
        self.card.refresh_from_db()
    ...
    def test_card_move_same_position(self):
        self.client.force_login(self.user)
        response = self.client.put(reverse("card_move"), {"card": self.card.id, "position": 0})
        self.assertEqual(response.status_code, status.HTTP_304_NOT_MODIFIED)
    ...
    def test_card_item_delete(self):
        self.client.force_login(self.user)
        response = self.client.delete(reverse("card_item_detail", kwargs={"pk": self.card_item_2.id}))
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
    ...
    def test_card_item_board_data_in_readonly_serializer(self):
        ...
        card_item_data = CardItemReadOnlySerializer(instance=card_item_with_board).data
    ...
    def test_card_item_get_log_label_comment_short(self):
        card_item = CardItem.objects.create(card=self.card, comment="Short comment")  # less than 50 char
        label = card_item.get_log_label()
    ...
    def test_board_log_list(self):
        ...
        response = self.client.get(reverse("board_log_list", kwargs={"pk": self.board.id}))
