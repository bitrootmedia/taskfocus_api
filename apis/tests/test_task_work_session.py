...
class TasksSessionTests(APITestCase):
    ...
    def test_task_start(self):
        ...
        response = self.client.post(reverse("task_start_work", kwargs={"pk": self.task_1.id}))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        # self.assertEqual(response.json().get("message"), "Testing Start")

    def test_task_stop(self):
        self.client.force_login(self.user)
        response = self.client.post(reverse("task_stop_work", kwargs={"pk": self.task_1.id}))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_update_task_session_stopped_at(self):
        ...
        self.client.patch(
            reverse("task_sessions_detail", kwargs={"pk": task_work_session_id}),
            data={"stopped_at": new_stopped_at},
        )
        tsd = TaskWorkSession.objects.get(pk=task_work_session_id)
        self.assertEqual(tsd.stopped_at.strftime("%Y-%m-%d %H:%M:%S"), new_stopped_at)
    ...
    def test_update_task_session_started_at(self):
        ...
        tsd = TaskWorkSession.objects.get(pk=task_work_session_id)
        self.assertEqual(tsd.started_at.strftime("%Y-%m-%d %H:%M:%S"), new_started_at)
    ...
    def test_task_total_time(self):
        ...
        response = self.client.get(reverse("task_total_time", kwargs={"pk": self.task_1.id}))
