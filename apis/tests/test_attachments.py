...
class AttachmentTests(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.task_1_project_1_user_1 = Task.objects.create(
            project=cls.project_1, owner=cls.user_1, title="Task 1"
        )

        cls.attachment_1_project_1_user_1 = Attachment.objects.create(
            project=cls.project_1, owner=cls.user_1
        )

        cls.attachment_2_project_2_user_2 = Attachment.objects.create(
            project=cls.project_2, owner=cls.user_2
        )

        cls.attachment_3_project_3_user_3 = Attachment.objects.create(
            project=cls.project_3, owner=cls.user_3
        )
    ...
    def test_attachment_no_access(self):
        ...
        self.assertNotIn(
            str(self.attachment_1_project_1_user_1.id), attachment_ids
        )

    def test_attachment_filter_project(self):
        self.client.force_login(self.user_1)
        response = self.client.get(
            reverse("attachment_list") + f"?project={self.project_3.id}"
        )
        attachment_ids = [
            x.get("id") for x in response.json().get("results", [])
        ]
        self.assertIn(
            str(self.attachment_3_project_3_user_3.id), attachment_ids
        )
        self.assertNotIn(
            str(self.attachment_1_project_1_user_1.id), attachment_ids
        )
    ...
    def test_update_own_attachment(self):
        ...
        self.assertEqual(
            updated_title, self.attachment_1_project_1_user_1.title
        )
